...
float4 main(pixel_shader_input input) : SV_TARGET
{
    ...
    float3 light_color = float3(0.35f, 1.0f, 0.9f); // light blue
    // float3 light_color = float3(1.0f, 1.0f, 1.0f);

    float3 light_position = float3(0.5f, -0.5f, 2.0f);
    ...
    float3 camera_direction = normalize(input.v_position.xyz - input.camera_pos);
